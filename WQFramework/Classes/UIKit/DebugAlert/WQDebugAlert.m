...
static NSMutableArray<WQDebugAlert *> *alertArray = nil;
...
- (void)dismissAlert {
    ...
    [self _dismiss:^(BOOL finished) {
        ...
        WQDebugAlert *alert = alertArray.firstObject;
        if (alert) {
            [alert _show:nil];
        }
    }];
...
- (DebugAlertFormatBlock)message {
    return ^WQDebugAlert *(NSString *format, ...) {
        NSString *text = nil;
        if (format) {
            va_list args;
...
- (DebugAlertStyleBlock)style {
    return ^WQDebugAlert *(DebugAlertStyle style) {
...
- (DebugAlertBoolBlock)autoDismiss {
    return ^WQDebugAlert *(BOOL autoDismiss) {
